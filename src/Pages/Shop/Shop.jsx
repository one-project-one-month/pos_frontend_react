...
const Shop = () => {
    ...
    const deleteShop = async () => {
        try {
            ...
            if (res.status === 200) {
                setIsDelete(false);
                setDatas((prevDatas) =>
                    prevDatas.filter((shop) => shop.id !== currentId)
                );
            }
        } catch (error) {
            console.log(error);
    ...
    return (
        <div>
            ...
            <div className="bg-gray-50 text-black h-[100vh] p-5">
                <div className="max-w-6xl mx-auto flex items-center flex-wrap gap-5">
                    {datas &&
                        datas.map((data, i) => (
                            <div
                                key={data.id}
                                className="bg-white border border-gray-200 rounded-md w-[280px] px-4 py-6 space-y-3"
                            >
